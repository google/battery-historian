...
def apply_fn_over_range(fn, start_time, end_time, arglist):
  ...
  Returns:
    A list of result generated by applying the function
    over the time range.
  """
  results = []
  cursor = start_time
...
class OverlapRec(object):
  """Record tracking events starting at the same time within a category."""
...
class Printer(object):
  ...
  # in _print_order are the same as category names.
  _print_order = [
      ...
      "activepower", "power"]
  # One-to-one mapped to _print_color. Category not specified
  # in _print_order will be given the default color.
  _print_color = [
  ...
  def aggregate_events(self, event_dict):
    ...
    Args:
      event_dict: A dict containing events.
...
class BHEmitter(object):
  ...
  def handle_event(self, t, e, timestr, event_dict, time_dict, highlight_dict):
    ...
    Args:
      t: Event time
      e: Event string
      timestr: Event time as string
      event_dict: A dict tracking events
      time_dict: A dict tracking wake_lock_in duration at different
